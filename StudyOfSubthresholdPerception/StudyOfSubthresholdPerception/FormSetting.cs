...
namespace StudyOfSubthresholdPerception
{
	public partial class FormSetting : Form
	{
			...
			switch (tabControl.SelectedIndex)
			{
				...
				case (int)Tabs.Experiment4:
					...
					try
					{
						...
						{
							dataGridView2.Rows.Add(item.Id, item.Img);
							dataGridView3.Rows.Add(item.Id, item.Img);
						}
						var data = ex4.GetData();
						foreach (var item in data)
						{
							dataGridView1.Rows.Add(item.Id, ex4.GetImageById(item.Id1).Img, ex4.GetImageById(item.Id2).Img);
						}
					}
					catch (Exception exp)
					{
		...
		private void buttonExp4AddPair_Click(object sender, EventArgs e)
		{
			...
			{
				var id1 = int.Parse(dataGridView2.SelectedRows[0].Cells[0].Value.ToString());
				var id2 = int.Parse(dataGridView3.SelectedRows[0].Cells[0].Value.ToString());
				var ex4 = new Experiment4DataHelper();
				var model = new Experiment4Model
				{
					Id1 = id1,
					Id2 = id2,
					ImageItem1 = ex4.GetImageById(id1).Img,
					ImageItem2 = ex4.GetImageById(id2).Img
				};
				dataGridView1.Rows.Add(new object[] { model.Id, model.ImageItem1, model.ImageItem2 });
				ex4.AddData(model);
			}
		}
		...
		private void button3_Click(object sender, EventArgs e)
		{
			var ex4 = new Experiment4DataHelper();
			var period = 0;
			int.TryParse(textBox1.Text, out period);
			var presentations = 0;
			int.TryParse(textBox2.Text, out presentations);
			ex4.SetSettings(new Experiment4SettingsModel { Period = period, Presentations = presentations });
		}
	}
}
